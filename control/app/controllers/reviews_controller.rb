...
class ReviewsController < ActionController::API
  ...
  def decide(state)
    change = Airlock::Review.decide!(Change.find(params[:id]), state: state, reviewer: @reviewer, reason: params[:reason])
    render json: summary(change)
  rescue Airlock::Review::Conflict => e
    render plain: e.message, status: :conflict
  end
  ...
  def authenticate_reviewer!
    ...
    return render(plain: "unauthorized", status: :unauthorized) unless Airlock::Review.token_ok?(given)
